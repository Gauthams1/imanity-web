     ...
     User.findOne({username:req.query.usr },function(err,us1){
       ...
       if (req.user) {
         Link.find({usrid:us1._id,type:"upload","details.data":(req.user._id+"#"+req.user.username)},function(err,us3){
           if(us3.length>0)
           res.render('tagview',{tagsout: us1,info1: us3,info2:us2,act:req.query.tag,layout:'usr2'});
           else {
             res.render('tagview',{tagsout: us1,act:req.query.tag,layout:'usr2'});
           }
         })
   ...
   if(req.query.del)
     {
	   ...
	   Tags.find({usrid:req.user._id},function(err,us2){
		res.render('tagview',{tagsout: us2 , tagath: req.user,hst: req.get('host')});
	  });});}});
//login get method
...
router.get('/tag', ensureAuthenticated, function(req, res){
	Tags.find({usrid: req.user._id},function(err,us1){//same tag can be used again for multiple action in tags
		User.find({},function(err,us2){                 //means with one name tag will do multiple actions
					res.render('tags',{ tagsin: us1,usrknn: us2});//<-- this will give you the complete list of people enroled
					});
	});
